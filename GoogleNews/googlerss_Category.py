...
# Logging information
today = datetime.now()
l = today.strftime("%b-%d-%Y")
d = today.strftime("%b-%d-%Y %H:%M:%S")

logFileName = 'log\DCHR_Scrape_Google_news_by_Category_' + l + '.log'
logging.basicConfig(filename=logFileName,filemode='a',level=logging.INFO,format='%(asctime)s :: %(levelname)s :: %(message)s')

logging.info("**************************************************************")
logging.info("Scrapping Google RSS News by Category at " + str(d))
logging.info("**************************************************************\n")
...
def ScrapeKeyword(keyword):
    ...
    json.dump(d, open("data\\Category\\"+keyword+".json","w"))
