...
logFileName = 'log\DCHR_Scrape_Google_news_by_Keyword_' + l + '.log'
logging.basicConfig(filename=logFileName,filemode='a',level=logging.INFO,format='%(asctime)s :: %(levelname)s :: %(message)s')

logging.info("**************************************************************")
logging.info("Scrapping Google RSS News by Keyword at " + str(d))
logging.info("**************************************************************\n")
...
def ScrapeKeyword(keyword):
    ...
    json.dump(d, open("data\\Keyword\\"+keyword+".json","w"))
